...
ALTER TABLE Usuarios MODIFY COLUMN fecha_ultimo_acceso DATETIME;
ALTER TABLE Usuarios MODIFY COLUMN fecha_ultimo_acceso DATETIME;
...
INSERT INTO Usuarios(id_rol,nombre,apellido,foto,email,clave,fecha_ultimo_acceso,esta_activo) 
	VALUES (4,"Roberto","Gomez","images/team/user.png","roberto@gmail.com",MD5("1234"),NOW(),1);

UPDATE Usuarios SET foto="images/team/team-3.png" WHERE id=7;
UPDATE Usuarios SET esta_activo=1 WHERE id=4;
UPDATE Usuarios SET fecha_ultimo_acceso=$fecha WHERE id=$id;

SELECT * FROM Usuarios;
SELECT id FROM Usuarios WHERE id=10;
...
ALTER TABLE Solicitudes MODIFY COLUMN fecha_carga DATETIME;
ALTER TABLE Solicitudes MODIFY COLUMN fecha_estimada_resolucion DATETIME;
ALTER TABLE Solicitudes MODIFY COLUMN id_usuario INT DEFAULT NULL;
SET @rol=2;
SET @id_s=7;
SELECT S.*
	FROM 
...
SELECT id_tipo, COUNT(id) AS conteo FROM Solicitudes GROUP BY id_tipo;
...
INSERT INTO Solicitudes(id_tipo,id_usuario,titulo,descripcion,fecha_carga,fecha_estimada_resolucion) 
	...
	IF (@tipo=1,DATE_ADD(NOW(),INTERVAL 7 DAY),
		IF (@tipo=2,DATE_ADD(NOW(),INTERVAL 3 DAY),
			IF (@tipo=3,DATE_ADD(NOW(),INTERVAL 1 DAY),NULL)
            )
		)
    );
